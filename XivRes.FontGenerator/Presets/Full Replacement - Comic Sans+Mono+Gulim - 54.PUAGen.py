...
try:
    ...
    fontforge = None
    print("No FontForge found: " + str(e))
...
class Matrix3x2:
    ...
    def create_skew(rad_x: float, rad_y: float, center: Vector2 = Vector2.ZERO):
        ...
        return Matrix3x2(1., y_tan, x_tan, 1., tx, ty)
...
class PathCubicBezierCurve:
    ...
    def transform(self, by: Matrix3x2):
        return PathCubicBezierCurve(self.p0.transform(by), self.p1.transform(by), self.p2.transform(by),
                                    self.p3.transform(by))
...
class PathArc:
    ...
    def __str__(self):
        return f"A {self.r} {self.angle} {'1' if self.large_arc else '0'} {'1' if self.sweep else '0'} {self.p1}"
...
class SvgPath:
    def __init__(self, d: str | None = None):
        ...
        coord2: Vector2 | None = None
        coord3: Vector2 | None = None
        coord4: Vector2 | None = None
        prevcmd = cmd = None
        ...
        try:
            while True:
                ...
                if what == 'Z' or what == 'z':
                    ...
                    self.__objects[-1].append(PathLine(coord, first))
                    coord = first
                ...
                if cmd == 'M' or cmd == 'm':
                    ...
                    if cmd == 'l':
                        coord2 += coord
                    self.__objects[-1].append(PathLine(coord, coord2))
                    coord = coord2
                elif cmd == 'H' or cmd == 'h':
                    ...
                    if cmd == 'c':
                        ...
                        coord3 += coord
                        coord4 += coord
                    self.__objects[-1].append(PathCubicBezierCurve(coord, coord2, coord3, coord4))
                    coord = coord4
                elif cmd == 'S' or cmd == 's':
                    ...
                    if cmd == 'q':
                        coord2 += coord
                        coord3 += coord
                    self.__objects[-1].append(PathQuadraticBezierCurve(coord, coord2, coord3))
                    coord = coord3
                elif cmd == 'T' or cmd == 't':
                    ...
                    if cmd == 'a':
                        coord2 += coord
                    self.__objects[-1].append(PathArc(coord, coord2, radius, angle, large_arc_flag, sweep_flag))
                elif cmd == 'Z' or cmd == 'z':
                    pass
                else:
                    print(cmd)
...
class OpenType:
    class OpenTypeView:
        MINIMUM_SIZE: typing.ClassVar[int]

        def __init__(self, v: bytes | bytearray | memoryview):
            if len(v) < self.__class__.MINIMUM_SIZE:
                raise ValueError
            self.v = memoryview(v)

    # noinspection PyPep8Naming,SpellCheckingInspection
    class TableDirectory(OpenTypeView):
        MINIMUM_SIZE = 4

        @property
        def sfntVersion(self): return int.from_bytes(self.v[0:4])

        @property
        def numTables(self): return int.from_bytes(self.v[4:6])

        @property
        def searchRange(self): return int.from_bytes(self.v[6:8])

        @property
        def entrySelector(self): return int.from_bytes(self.v[8:10])

        @property
        def rangeShift(self): return int.from_bytes(self.v[10:12])

        @property
        def tableRecords(self): return [
            OpenType.TableRecord(self.v[12 + i * OpenType.TableRecord.MINIMUM_SIZE:])
            for i in range(self.numTables)]

    class TableRecord(OpenTypeView):
        MINIMUM_SIZE = 16

        @property
        def tag(self): return self.v[0:4]

        @property
        def checksum(self): return int.from_bytes(self.v[4:8])

        @property
        def offset(self): return int.from_bytes(self.v[8:12])

        @property
        def length(self): return int.from_bytes(self.v[12:16])

    class Test:
        def __init__(self):
            OpenType.TableDirectory().numTables = 3
...
round_box_hexagon = InverseBackground(-10, 0, 1260, 1000, SvgPath(
    "M417 999h425q55 -1 110 -34q54 -33 82 -80l159 -279q26 -49 26 -111q0 -63 -26 -111l-159 -276q-28 -47 -82 -77 q-55 -30 -110 -31h-425q-55 1 -109 31q-55 30 -83 77l-159 275q-26 48 -26 111q0 64 26 112l159 278q28 48 83 81q54 33 109 34v0z"))
round_box_square = InverseBackground(-10, 0, 1010, 1000, SvgPath(
    "M816 923q46 -1 77 -31q30 -30 32 -76v-328v-304q-2 -46 -32 -77q-31 -31 -77 -32h-632q-46 1 -77 32t-32 77v632q1 46 32 76t77 31h632z"))
round_box_square_wide = InverseBackground(-10, 0, 1760, 1000, SvgPath(
    "M1382 50h-1014q-85 2 -142 59q-56 56 -58 141v500q2 85 58 141q57 56 142 58h1014q85 -2 142 -58q56 -56 58 -141v-500q-2 -85 -58 -141q-57 -57 -142 -59v0z"))
round_box_square_narrow = InverseBackground(-10, 0, 760, 1000, SvgPath(
    "M151 917h438q47 -1 78 -31t32 -77v-638q-1 -46 -32 -78q-31 -31 -78 -32h-438q-47 1 -78 32q-31 32 -32 78v638q1 47 32 77t78 31v0z"))
round_box_square_wide2 = InverseBackground(-10, 0, 2010, 1000, SvgPath(
    "M1703 968q99 -3 166 -69q66 -66 69 -165v-468q-3 -99 -69 -166q-67 -66 -166 -69h-1406q-99 3 -165 69q-66 67 -69 166v468q3 99 69 165t165 69h1406z"))
# <svg width="1000" height="1000" xmlns="http://www.w3.org/2000/svg">
round_box_square_outline = InverseBackground(-10, 0, 1010, 1000, SvgPath(
    "M 190,45 C 110.10243,45 45,110.10243 45,190 v 620 c 0,79.89757 65.10243,145 145,145 h 620 c 79.89757,0 145,-65.10243 145,-145 V 190 C 955,110.10243 889.89757,45 810,45 Z m 0,50 h 620 c 53.06243,0 95,41.93757 95,95 v 620 c 0,53.06243 -41.93757,95 -95,95 H 190 C 136.93757,905 95,863.06243 95,810 V 190 c 0,-53.06243 41.93757,-95 95,-95 z").reverse())
...
gen_map: list[GenDef] = [
    ...
    GenDef(
        "AM/PM",
        ['<tspan x="0">A</tspan><tspan x="0" dy="0.8em">M</tspan>',
         '<tspan x="0">P</tspan><tspan x="0" dy="0.8em">M</tspan>'],
        0xE06D,
        ...
        for gen_item in gen_map:
            ...
            for c in gen_item.items:
                ...
                with open(filename, "w", encoding="utf-8") as fp:
                    ...
                    fp.write(
                        f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="{gen_item.view_box}">\n')
